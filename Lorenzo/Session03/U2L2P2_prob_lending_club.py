...
coltitles = [col.replace("."," ") for col in colname] #SF# Convert to better names

sns.boxplot(data, names=coltitles, color='pastel')
plt.savefig("boxplot.pdf", bbox_inches='tight')
plt.title('Boxplot')
plt.show()

# Violinplot, allowing to compare if two different data set have same distribution
sns.violinplot(data, names=coltitles, color='pastel')
...
for i in range(2):
	...
	axs[i].set_title(coltitles[i],fontsize=13)
	axs[i].set_xlabel('')
...
plt.suptitle("QQplot against the normal distribution") #SF# Suptitle to the rescue
plt.savefig("qqplot-normal.pdf", bbox_inches='tight')
plt.show()
...
for i in range(2):
	fig.add_subplot(1,2,i+1)
	stats.probplot(data[i], dist='uniform', plot=plt)
plt.suptitle("QQplot against the uniform distribution") #SF# Suptitle to the rescue
plt.savefig("qqplot-uniform.pdf", bbox_inches='tight')
plt.show()
...
# SF # The distribution is not normal! Uniform is another kind of distribution, beware what you are doing
